...
def find_contrast_and_brightness1(D, S):
    #luminosité adaptée au contraste fixé
    contrast = 0.75
...
### Compression image en niveau de gris
...
def generate_all_transformed_blocks(img, source_size, destination_size, step):
    ...
    for k in range((img.shape[0] - source_size) // step + 1):
        for l in range((img.shape[1] - source_size) // step + 1):
            #réduit un bloc source à la forme d'un bloc destination
            S = reduce(img[k*step:k*step+source_size,l*step:l*step+source_size], factor)
            #génère tous les blocs transformés possibles
...
def compress(img, source_size, destination_size, step):
    ...
    for i in range(i_count):
        ...
        for j in range(j_count):
            ...
            min_d = float('inf')
            #récupère le bloc destination
            D = img[i*destination_size:(i+1)*destination_size,j*destination_size:(j+1)*destination_size]
            #teste toutes les transformations possibles et garde la meilleure
...
def test_greyscale():
    img = mpimg.imread('chemin vers l'image'')
